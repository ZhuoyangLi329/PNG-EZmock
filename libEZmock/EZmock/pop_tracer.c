...
/******************************************************************************
Function `apply_png_shift`:
  示踪物层面 PNG 注入（2026-09-21 新增）：把注入位移场 mesh->inj[0..2]
  （由 perturb.c 计算，节点采样的实空间场，单位 Mpc/h）用三线性插值加到每个
  示踪物位置上（周期性边界）。位移场只在 fnl≠0 且 b_phi≠0 时分配，
  为 NULL 时调用方直接跳过本函数（输出与历史版本逐位一致）。

  约定与生成端一致：场值放在整数格点 i·gs 上（同 psi 的用法），
  插值取 i0=floor(x/gs) 与 i0+1 两个节点。
Arguments:
  * `ez`:       instance of the EZmock generator;
  * `x`:        array of the x coordinates (updated in place);
  * `y`:        array of the y coordinates (updated in place);
  * `z`:        array of the z coordinates (updated in place);
  * `nt`:       number of tracers.
******************************************************************************/
static void apply_png_shift(EZMOCK *ez, real *x, real *y, real *z,
    const size_t nt) {
  EZMOCK_CONF *conf = (EZMOCK_CONF *) ez->conf;
  EZMOCK_MESH *mesh = (EZMOCK_MESH *) ez->mesh;
  const int Ng = conf->Ng;
  const real Lbox = conf->Lbox;
  const real gfac = (real) Ng / Lbox;       /* Mpc/h -> 格子坐标 */
  const FFT_REAL *fx = mesh->inj[0], *fy = mesh->inj[1], *fz = mesh->inj[2];

  /* 每个示踪物独立、写入位置互不相干，可以并行且结果确定。 */
#ifdef OMP
#pragma omp parallel for num_threads(conf->nthread)
#endif
  for (size_t i = 0; i < nt; i++) {
    /* 位置 -> 格子坐标，floor 后对 Ng 取模（浮点上贴边位置可能刚好越界） */
    const real u = x[i] * gfac, v = y[i] * gfac, w = z[i] * gfac;
    const real fu = u - floor(u), fv = v - floor(v), fw = w - floor(w);
    int i0 = (((int) floor(u) % Ng) + Ng) % Ng;
    int j0 = (((int) floor(v) % Ng) + Ng) % Ng;
    int k0 = (((int) floor(w) % Ng) + Ng) % Ng;
    const int ix[2] = {i0, (i0 + 1) % Ng};
    const int iy[2] = {j0, (j0 + 1) % Ng};
    const int iz[2] = {k0, (k0 + 1) % Ng};
    const real wx[2] = {(real) 1 - fu, fu};
    const real wy[2] = {(real) 1 - fv, fv};
    const real wz[2] = {(real) 1 - fw, fw};
    ...
    for (int a = 0; a < 2; a++) {
      for (int b = 0; b < 2; b++) {
        for (int c = 0; c < 2; c++) {
          const real wgt = wx[a] * wy[b] * wz[c];
          const size_t idx = IDX(Ng, ix[a], iy[b], iz[c]);
          sx += wgt * fx[idx];
          sy += wgt * fy[idx];
          sz += wgt * fz[idx];
        }
    ...
    /* 加位移并回卷到 [0, Lbox)（位移幅度 ≪ Lbox，单次修正足够） */
    x[i] += sx;
    y[i] += sy;
    z[i] += sz;
    if (x[i] < 0) x[i] += Lbox; else if (x[i] >= Lbox) x[i] -= Lbox;
    if (y[i] < 0) y[i] += Lbox; else if (y[i] >= Lbox) y[i] -= Lbox;
    if (z[i] < 0) z[i] += Lbox; else if (z[i] >= Lbox) z[i] -= Lbox;
  }
}
...
int EZmock_populate_tracer(EZMOCK *ez, const real *params, const size_t nexp,
  ...
  /* 示踪物层面 PNG 注入：给位置加一个由势场决定的位移（关闭时 inj[0]=NULL，
     逐位回退到历史行为）。速度保持 ZA 的速度场不额外修改。 */
  if (mesh->inj[0] && mesh->inj[1] && mesh->inj[2])
    apply_png_shift(ez, *x, *y, *z, pdf->ntot);
