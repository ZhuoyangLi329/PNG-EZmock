...
static int EZmock_disp_with_rng(EZMOCK *ez, const bool fixamp,
  ...
  FFT_PLAN plan2 = FFT_PLAN_R2C(conf->Ng, conf->Ng, conf->Ng,
     mesh->phi_png, mesh->phik_png, FFT_FLAG);;

  EZMOCK_PK *pk = (EZMOCK_PK *) ez->pk;
  if (pk->log) {
    if (fixamp) {
      if (iphase) EZmock_ZA_disp_logpk_fixamp_iphase(ez, plan, plan2,err);
      else EZmock_ZA_disp_logpk_fixamp(ez, plan, plan2,err);
    }
    else {
      if (iphase) EZmock_ZA_disp_logpk_iphase(ez, plan, plan2,err);
      else EZmock_ZA_disp_logpk(ez, plan, plan2,err);
    }
  }
  else {
    if (fixamp) {
      if (iphase) EZmock_ZA_disp_fixamp_iphase(ez, plan, plan2,err);
      else EZmock_ZA_disp_fixamp(ez, plan, plan2,err);
    }
    else {
      if (iphase) EZmock_ZA_disp_iphase(ez, plan, plan2,err);
      else EZmock_ZA_disp(ez, plan, plan2,err);
    }
  }
...
int EZmock_create_dens_field(EZMOCK *ez, real *psi[3], bool deepcopy,
  ...
  if (psi && psi[0] && psi[1] && psi[2]) {      /* set the displacements */
    ...
    if (!(mesh->psi[0] || (mesh->psi[0] = FFT_MALLOC(size))) ||
        ...
        !(mesh->phik || (mesh->phik = FFT_MALLOC(csize))) ||
        !(mesh->phi || (mesh->phi = FFT_MALLOC(size))) ||

        !(mesh->phik_png || (mesh->phik_png = FFT_MALLOC(csize))) ||
        !(mesh->phi_png || (mesh->phi_png = FFT_MALLOC(size))) ||
        

        !(mesh->rhok || (mesh->rhok = FFT_MALLOC(csize))) ||
        !(mesh->rhok3 || (mesh->rhok3 = FFT_MALLOC(csize))) ||
        !(mesh->rhok4 || (mesh->rhok4 = FFT_MALLOC(csize))) ||
        !(mesh->rhok2 || (mesh->rhok2 = FFT_MALLOC(csize)))) {
      return (*err = EZMOCK_ERR_MEMORY);
    }
    mesh->psi_ref = false;

    /* ---- 示踪物层面 PNG 注入的位移场（2026-09-21 新增）----
       仅在 fnl≠0 且 b_phi≠0 时分配（此时 perturb.c 会往里填注入位移，
    ...
    if (delta)
    {
      ...
      EZmock_disp_with_whitenoise(ez, delta, err);
    }
    else 
    {
      
      EZmock_disp_with_rng(ez, fixamp, iphase, err);
