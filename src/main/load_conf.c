...
INVERT_PHASE    = \n\
    ...
    # Gaussian random field (unset: %c).\n\
FNL             = \n\
    # Double-precision number, local PNG parameter fNL of the initial\n\
    # potential (unset: "
...
FNL_FIELD       = \n\
    ...
    # unset: equal to FNL if B_PHI = 0 (historical behavior); otherwise 0\n\
    # (so that the calibrated tracer-level injection alone controls f_NL).\n\
\n\
##################################################\n\
...
VERBOSE         = \n\
  ...
  (double) DEFAULT_FNL, (double) DEFAULT_B_PHI,
...
static cfg_t *conf_read(CONF *conf, const int argc, char *const *argv) {
  ...
  const cfg_param_t params[] = {
    ...
    {'f', "fnl"          , "FNL"            , CFG_DTYPE_DBL , &conf->fnl     },
    { 0 , "b-phi"        , "B_PHI"          , CFG_DTYPE_DBL , &conf->b_phi   },
    { 0 , "fnl-field"    , "FNL_FIELD"      , CFG_DTYPE_DBL , &conf->fnl_field},
...
static int conf_verify(const cfg_t *cfg, CONF *conf) {
  ...
  if (!isfinite(conf->b_phi)) {
    P_ERR(FMT_KEY(B_PHI) " must be finite\n");
    return EZMOCK_ERR_CFG;
  }
  if (conf->b_phi != 0.0 && conf->fnl == 0.0)
    P_WRN(FMT_KEY(B_PHI) " is non-zero but " FMT_KEY(FNL) " is 0: "
        "Ainj = 2*FNL*B_PHI = 0, no tracer-level injection\n");

  /* Check FNL_FIELD（场层面二次项系数，可选）.
     未设置时：B_PHI=0（历史模式）取 FNL，逐位保持旧行为；B_PHI≠0（标定模式）
     取 0，由示踪物层面注入单独控制 f_NL，避免未标定的旧机制混进来. */
  if (!cfg_is_set(cfg, &conf->fnl_field))
    conf->fnl_field = (conf->b_phi != 0.0) ? 0.0 : conf->fnl;
  if (!isfinite(conf->fnl_field)) {
    P_ERR(FMT_KEY(FNL_FIELD) " must be finite\n");
    return EZMOCK_ERR_CFG;
  }
...
static void conf_print(const CONF *conf) {
  ...
  printf("\n  FNL             = " OFMT_DBL, conf->fnl);
  printf("\n  B_PHI           = " OFMT_DBL, conf->b_phi);
  printf("\n  FNL_FIELD       = " OFMT_DBL, conf->fnl_field);
