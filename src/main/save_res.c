...
static int save_box_ascii(CONF *conf, const real *x, const real *y,
  ...
  if (conf->header) {
    WRITE_LINE(ofile, "%c BOX_SIZE=" OFMT_DBL " , NUM_GRID=%d , "
        "FIX_AMPLITUDE=%c , INVERT_PHASE=%c , FNL=" OFMT_DBL "\n",
        EZMOCK_SAVE_COMMENT, conf->Lbox, conf->Ngrid,
        conf->fixamp ? 'T' : 'F', conf->iphase ? 'T' : 'F', conf->fnl);
    WRITE_LINE(ofile, "%c B_PHI=" OFMT_DBL " , FNL_FIELD=" OFMT_DBL "\n",
        EZMOCK_SAVE_COMMENT, conf->b_phi, conf->fnl_field);
...
static int save_chunk_fits(CONF *conf, const char *fname, real *x, real *y,
  ...
  if (conf->header) {
    ...
    if (fits_write_key(fp, TLONGLONG, "NUM_TOT" , &ntracer,
        ...
        fits_write_key(fp, TDOUBLE  , "FNL"     , &conf->fnl,
            "local PNG parameter fNL of the initial potential"  , &status) ||
        fits_write_key(fp, TDOUBLE  , "B_PHI"   , &conf->b_phi,
            "tracer response of the tracer-level PNG injection" , &status) ||
        fits_write_key(fp, TDOUBLE  , "FNL_FLD" , &conf->fnl_field,
            "coefficient of the field-level quadratic term"     , &status) ||
